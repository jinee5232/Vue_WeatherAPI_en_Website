    ...
    >
      <g class="counties">
        <path
          ...
          :d="county.path"
          :class="['county', { 
            'active': activeCounty === county.name,
            'available': availableCounties.includes(county.name)
          }]"
          @click="availableCounties.includes(county.name) && $emit('select', county.name)"
        >
      ...
      <g v-if="showLabels" class="labels">
        <text
          v-for="label in countyLabels"
          :key="label.id"
          :x="label.x"
          :y="label.y"
          class="map-label"
          @click="availableCounties.includes(label.name) && $emit('select', label.name)"
        >
          {{ getTempForCounty(label.name) }}
        </text>
      </g>
    </svg>
  </div>
...
export default {
  ...
  data() {
    return {
      counties: REAL_TAIWAN_DATA
    }
  },
  computed: {
    countyLabels() {
      return this.counties.map(c => {
        const bbox = this.getApproxCentroid(c.id);
        return { ...c, ...bbox };
      });
    }
  },
  methods: {
    getTempForCounty(name) {
      const temp = this.countyTemps[name];
      return temp ? `${Math.round(temp)}°` : '';
    },
    getApproxCentroid(id) {
      const centroids = {
        'taipei-city': { x: 897, y: 400 },
        'new-taipei-city': { x: 915, y: 450 },
        'keelung-city': { x: 934, y: 350 },
        'taoyuan-city': { x: 861, y: 480 },
        'hsinchu-county': { x: 843, y: 550 },
        'hsinchu-city': { x: 724, y: 480 },
        'miaoli-county': { x: 724, y: 560 },
        'taichung-city': { x: 803, y: 640 },
        'changhua-county': { x: 647, y: 720 },
        'nantou-county': { x: 825, y: 750 },
        'yunlin-county': { x: 642, y: 800 },
        'chiayi-county': { x: 650, y: 880 },
        'chiayi-city': { x: 573, y: 840 },
        'tainan-city': { x: 550, y: 920 },
        'kaohsiung-city': { x: 739, y: 1000 },
        'pingtung-county': { x: 700, y: 1150 },
        'yilan-county': { x: 982, y: 550 },
        'hualien-county': { x: 935, y: 750 },
        'taitung-county': { x: 889, y: 1000 },
        'penghu-county': { x: 320, y: 900 },
        'kinmen-county': { x: 50, y: 550 },
        'lienchiang-county': { x: 469, y: 100 }
      };
      return centroids[id] || { x: 0, y: 0 };
    }
  }
}
</script>
